...
# hsieh: add gaussian_splatting. in front of utils to adapt to new repo structure
import torch
import numpy as np
...
from gaussian_splatting.scene.regulation import compute_plane_smoothness
from typing import Tuple

class XGaussianModel:
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale : float, time_line: int):
        ...
        # hsieh: modified for flexible deformation model in s3pogs
        weight_coefs = torch.zeros((N, self.args["deform_gs"]["ch_num"], self.args["deform_gs"]["curve_num"]))
        position_coefs = torch.zeros((N, self.args["deform_gs"]["ch_num"], self.args["deform_gs"]["curve_num"])) + torch.linspace(0,1,self.args["deform_gs"]["curve_num"])
        shape_coefs = torch.zeros((N, self.args["deform_gs"]["ch_num"], self.args["deform_gs"]["curve_num"])) + self.args["deform_gs"]["init_param"]
