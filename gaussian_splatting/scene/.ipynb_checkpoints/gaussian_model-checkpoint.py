...
class GaussianModel:
    ...
    # 处理输入的相机信息和图像数据，然后调用create_pcd_from_image_and_depth生成点云
    def create_pcd_from_image(self, cam_info, init=False, scale=2.0, depthmap=None):
        cam = cam_info
        image_ab = (torch.exp(cam.exposure_a)) * cam.original_image + cam.exposure_b    # 计算图像的曝光校正，dust3r前要不要加
    ...
    # 基于rbg和depth，使用open3d创建点云、随机下采样，并保存点云信息为BasicPointCloud类，然后完成3DGS的初始化
    def create_pcd_from_image_and_depth(self, cam, rgb, depth, init=False):
        ...
        #print("before 下采样 点云个数：", len(pcd_tmp.points))
        ...
        #print(new_rgb[:3])
        #o3d.io.write_point_cloud('/media/ysc/Data/3DGS/project/MonoGS/pts/original_pcd.ply', pcd_tmp)
        pcd = BasicPointCloud(
            points=new_xyz, colors=new_rgb, normals=np.zeros((new_xyz.shape[0], 3))
        )
        self.ply_input = pcd

        fused_point_cloud = torch.from_numpy(np.asarray(pcd.points)).float().cuda()     # 点云
        fused_color = RGB2SH(torch.from_numpy(np.asarray(pcd.colors)).float().cuda())   # 通过颜色得到球谐系数
        features = (
        ...
        rots[:, 0] = 1
        opacities = inverse_sigmoid(            # 使用逆 sigmoid 函数初始化不透明度 opacities
            0.5
            * torch.ones(
    ...
    # 将来自DUST3R的点云和颜色进行下采样，保存为BasicPointCloud类，然后进行3DGS初始化
    def create_pcd_from_dust3r(self, pts3d, imgs, T, frame_idx, save_dir, scale=1, mask=None, init=False ):
        ...
        pcd_dir = os.path.join(save_dir, "pcd_相机坐标系")
        if not os.path.exists(pcd_dir):
            os.makedirs(pcd_dir)
        save_path = os.path.join(pcd_dir, f"{frame_idx}_初始点云.ply")
        o3d.io.write_point_cloud(save_path, pcd)
        print(f"初始点云已保存至: {save_path}")
        
        T = T.detach().cpu().numpy()
        pcd.transform(T)
        
        pcd_dir = os.path.join(save_dir, "pcd_世界坐标系")
        if not os.path.exists(pcd_dir):
            os.makedirs(pcd_dir)
        save_path = os.path.join(pcd_dir, f"{frame_idx}_初始点云.ply")
        o3d.io.write_point_cloud(save_path, pcd)
        print(f"初始点云已保存至: {save_path}")
        #o3d.io.write_point_cloud('/media/ysc/Data/3DGS/project/MonoGS/pts/100613_pcd_before_downsample.ply', pcd)
        ...
        #o3d.io.write_point_cloud('/media/ysc/Data/3DGS/project/MonoGS/pts/100613_pcd_after_downsample稀疏.ply', pcd)
        new_xyz = np.asarray(pcd.points)
        new_rgb = np.asarray(pcd.colors)
        print("after 下采样 xyz's shape is:",new_xyz.shape)
        #print(new_xyz[:3])
        ...
        fused_point_cloud = torch.from_numpy(new_xyz).float().cuda()     # 点云
        fused_color = RGB2SH(torch.from_numpy(new_rgb).float().cuda())  # 通过颜色得到球谐系数
        features = (
            torch.zeros((fused_color.shape[0], 3, (self.max_sh_degree + 1) ** 2))
            .float()
            .cuda()
    ...
    # 通过上面初始化得到的gaussian点，转为可训练的参数，然后调用densification_postfix加入当前场景
    def extend_from_pcd(
