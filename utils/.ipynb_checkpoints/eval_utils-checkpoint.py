...
#print("Current backend:", matplotlib.get_backend())  # 应该输出 'Agg'
import cv2
import evo
import numpy as np
import torch
...
# 使用 evo 工具评估估计轨迹与Ground Truth轨迹的对齐情况，计算并绘制轨迹的ATE
...
# 评估渲染图像的质量，包括 PSNR、SSIM 和 LPIPS，这里竟然每隔5个帧且不是关键帧才eval
def eval_rendering(
    frames,
    gaussians,
    dataset,
    ...
    for idx in range(0, end_idx, interval):
        ...
        residual = np.clip(residual, 0, 255).astype(np.uint8)  # 确保值在合法范围内
        ...
        plt.subplot(2, 2, 3)
        plt.imshow(img_depth, cmap='gray')  # 假设深度图是灰度图
        plt.title('Depth Map')
        plt.axis('off')
